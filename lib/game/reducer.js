...
function setFullHouse (state, action) {
  ...
  const uniqueDice = unique(state.dice).length
  const dieCount = count(state.dice, state.dice[0])
  const newValue = uniqueDice === 2 && (dieCount === 3 || dieCount === 2) ? 25 : 0
...
function setSmallStraight (state, action) {
  ...
  const newValue = isStraight(state.dice, 4) ? 30 : 0
  return {
    ...state,
    held: initialState.held,
    scores: {
...
function setLargeStraight (state, action) {
  ...
  const newValue = isStraight(state.dice, 5) ? 40 : 0
  return {
    ...state,
    held: initialState.held,
    scores: {
...
function setChance (state, action) {
  ...
  return {
    ...
    scores: {
      ...state.scores,
      chance: scored(state.dice.reduce((acc, x) => acc + x))
    },
    throwsLeft: initialState.throwsLeft
  }
}

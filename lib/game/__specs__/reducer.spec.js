...
describe('game/reducer', () => {
  describe('/initial state', () => {
    it('sets scores to zeros and not scored', () => {
      ...
      scores.aces.should.eql({ scored: false, value: 0 })
      scores.twos.should.eql({ scored: false, value: 0 })
      scores.threes.should.eql({ scored: false, value: 0 })
      scores.fours.should.eql({ scored: false, value: 0 })
      scores.fives.should.eql({ scored: false, value: 0 })
      scores.sixes.should.eql({ scored: false, value: 0 })
      scores.upperSubTotal.should.eql(0)
      scores.upperBonus.should.eql(0)
      scores.upperTotal.should.eql(0)
      scores.threeOfAKind.should.eql({ scored: false, value: 0 })
      scores.fourOfAKind.should.eql({ scored: false, value: 0 })
      scores.fullHouse.should.eql({ scored: false, value: 0 })
      scores.smallStraight.should.eql({ scored: false, value: 0 })
      scores.largeStraight.should.eql({ scored: false, value: 0 })
      scores.dinkzee.should.eql({ scored: false, value: 0 })
      scores.chance.should.eql({ scored: false, value: 0 })
  ...
  describe('SET_UPPER_VALUE', () => {
    ...
    it('should give bonus when >= 63 subtotal', () => {
      const state = {
        ...
        scores: {
          ...initialState.scores,
          fours: { scored: true, value: 16 },
          fives: { scored: true, value: 20 },
          sixes: { scored: true, value: 24 }
        }
      }
      const action = { type: TYPES.SET_UPPER_VALUE, selected: 'threes' }
      const nextState = reducer(state, action)
  ...
  describe('SET_3_OF_A_KIND', () => {
    ...
    it('should ignore scoring and not reset when already scored', () => {
      const state = {
        ...
        scores: {
          ...initialState.scores,
          threeOfAKind: { scored: true, value: 0 }
        },
        throwsLeft: 0
      }
      const nextState = reducer(state, { type: TYPES.SET_3_OF_A_KIND })
  ...
  describe('SET_4_OF_A_KIND', () => {
    ...
    it('should ignore scoring and not reset when already scored', () => {
      const state = {
        ...
        scores: {
          ...initialState.scores,
          fourOfAKind: { scored: true, value: 0 }
        },
        throwsLeft: 0
      }
      const nextState = reducer(state, { type: TYPES.SET_4_OF_A_KIND })
  ...
  describe('SET_SMALL_STRAIGHT', () => {
    ...
    it('should ignore scoring and not reset when already scored', () => {
      const state = {
        ...
        scores: {
          ...initialState.scores,
          smallStraight: { scored: true, value: 0 }
        },
        throwsLeft: 0
      }
      const nextState = reducer(state, { type: TYPES.SET_SMALL_STRAIGHT })
  ...
  describe('SET_LARGE_STRAIGHT', () => {
    ...
    it('should ignore scoring and not reset when already scored', () => {
      const state = {
        ...
        scores: {
          ...initialState.scores,
          largeStraight: { scored: true, value: 0 }
        },
        throwsLeft: 0
      }
      const nextState = reducer(state, { type: TYPES.SET_LARGE_STRAIGHT })
  ...
  describe('SET_CHANCE', () => {
    ...
    it('should add up lower totals on set', () => {
      const state = {
        ...
        scores: {
          ...initialState.scores,
          threeOfAKind: { scored: true, value: 5 }
        }
      }
